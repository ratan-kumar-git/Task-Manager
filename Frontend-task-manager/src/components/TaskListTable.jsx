  ...
  return (
    <div className="overflow-x-auto p-0 rounded-lg mt-3">
      <table className="min-w-full">
        <thead>
          <tr className="text-left text-gray-800 font-medium text-[13px]">
            <th className="px-4 py-3">Name</th>
            <th className="px-4 py-3">Status</th>
            <th className="px-4 py-3">Priority</th>
            <th className="px-4 py-3 hidden md:table-cell">Created On</th>
          </tr>
        </thead>
        <tbody>
          {tableData.map((task) => (
            <tr key={task._id} className="border-t border-gray-200">
              ...
              <td className="px-4 py-4">
                <span
                  className={`px-2 py-1 text-xs inline-block ${getStatusBadgeColor(
                    task.status
                  )}`}
                >
                  {task.status}
              ...
              <td className="px-4 py-4">
                <span
                  className={`px-2 py-1 text-xs inline-block ${getPriorityBadgeColor(
                    task.priority
                  )}`}
                >
                  {task.priority}
